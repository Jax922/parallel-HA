...
scanIncBlock(volatile typename OP::RedElTp* ptr, const unsigned int idx) {
    ...
    if (lane == (WARP-1)) {
        if (warpid < 31) {
            typename OP::RedElTp tmpVal = OP::remVolatile(ptr[idx]);
            ptr[warpid] = tmpVal;
        } else if (warpid == 30) {
            typename OP::RedElTp tmpVal1 = OP::remVolatile(ptr[idx]);
            typename OP::RedElTp tmpVal2 = OP::remVolatile(ptr[idx+1]);
            ptr[warpid] = OP::apply(tmpVal1, tmpVal2);
        }
    }
    __syncthreads();
    ...
    if (warpid > 0) {
        typename OP::RedElTp temp = OP::remVolatile(ptr[warpid-1]);
        res = OP::apply(temp, res);
    }
