...
__global__ void mmmSymBlkRegInnSeqKer(ElTp* A, ElTp* B, ElTp* C, int heightA, int widthB, int widthA) {
  ...
  for(int kk = 0; kk < widthA; kk += Tk) {
      ...
      for (int i = 0; i < Ry; ++i) {
        int row_idx = iii + threadIdx.y * Ry + i;
        int col_idx = kk + threadIdx.x;
        Aloc[threadIdx.y * Ry + i][threadIdx.x] = (row_idx < heightA && col_idx < widthA) ? A[row_idx * widthA + col_idx] : 0;
      }
      ...
      for (int i = 0; i < Rx; ++i) {
          int row_idx = kk + threadIdx.y;
          int col_idx = jjj + threadIdx.x * Rx + i;
          Bloc[threadIdx.y][threadIdx.x * Rx + i] = (row_idx < widthA && col_idx < widthB) ? B[row_idx * widthB + col_idx] : 0;
      }
      __syncthreads();
