...
void multiStepHisto ( uint32_t* d_inp_inds
     ...
     ****************************************************************/
    // Loop through partitions to process histogram segments separately.
    for (uint32_t i = 0; i < num_partitions; i++) {
        // Get the bounds (lower and upper)
        uint32_t lower = i * CHUNK;
        uint32_t upper = min( (i+1) * CHUNK, H);
        multiStepKernel<<<grid,B>>>(d_inp_inds, d_inp_vals, d_hist, N, lower, upper);
    }
